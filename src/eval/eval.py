...
def eval_single_dataset(image_encoder, dataset_name, args):
    ...
    model.eval()
    print(dataset_name,args.data_location)
    dataset = get_dataset(
        dataset_name,
...
def evaluate_task_vector(
    ...
    if args.method.name == "tall_mask" or eval_masks is not None:
        ...
        scaling_coef_range = np.linspace(0.03263157894736842,0.04, args.n_eval_points)
        print("Evaluation", scaling_coef_range)
    else:
        scaling_coef_range = np.linspace(0.7, 1.0, args.n_eval_points)[1:]
